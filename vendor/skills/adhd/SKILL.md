...
license: MIT
when-to-use: "Use only for difficult divergent decisions, fuzzy debugging, API/schema alternatives, or trap detection. Skip ordinary CRUD, typos, and bugs with a known cause."
...
2. Spawn 5 **parallel** `spawn_subagent` calls. One per frame. Each Agent
   gets only:
   - the problem P
