...
Choose one profile from the changed surface and the risk. Discover commands from the current repository. A missing command is `NOT_CONFIGURED`, not success. Required configured failures block a completion claim.
...
Run project-native typecheck and relevant tests, then the repository's configured browser or E2E workflow. Use `/browser-act` to discover issues. Use `/chrome-devtools-axi` only after an observed failure. Both attach through background Chromium (`grok-chromium-cdp`); follow the browser engine contract in `00-routing.md`. Use project Playwright to lock a repaired critical flow. Do not initialize browser tooling that the project does not already have.
